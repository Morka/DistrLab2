...
public class SimpleTest {
	...
	public void before() throws Exception {
		...
		server = componentFactory
				.startFileServer(new Config("fs1"),
						new Shell("fs1", new TestOutputStream(System.out),
								new TestInputStream()));
		Thread.sleep(Util.WAIT_FOR_COMPONENT_STARTUP);

		client = componentFactory.startClient(new Config("client"), new Shell(
	...
	public void after() throws Exception {
		...
		try {
			...
			// This should not happen. In case it does, output the stack trace
	...
	public void test() throws Exception {
		...
		assertTrue(String.format("Response must contain '%s' but was '%s'",
		...
		assertTrue(String.format("Response must start with '%s' but was '%s'",
		...
		expected = "Uploaded file to server, new Credits: 298";
		assertTrue(String.format("Response must contain '%s' but was '%s'",
				expected, actual), actual.contains(expected));

		actual = client.credits().toString();
		expected = "!credits 298";
